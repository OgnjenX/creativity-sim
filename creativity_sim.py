...
class MemoryBuffers:
    ...
    def sample_pair(
        self, alpha: float, alpha_replay_thresh: float = 0.4
        ...
        p_replay = self._compute_replay_preference(
            alpha, alpha_replay_thresh, have_replay
        )
        prefer_replay = bool((torch.rand(()) < p_replay).item())
        use_replay_first = have_replay and (not have_live or prefer_replay)
...
def _select_pair_or_fallback(
    buffers: MemoryBuffers, curr_alpha: float, alpha_replay_thresh: float, dim: int
) -> tuple[Tensor, Tensor]:
    pair = buffers.sample_pair(curr_alpha, alpha_replay_thresh=alpha_replay_thresh)
    if pair is None:
...
def main():
    ...
    for step in range(n_steps):
        curr_alpha = _update_alpha_for_step(alpha_ctrl, step, creativity_log)

        x_i, x_j = _select_pair_or_fallback(
            buffers, curr_alpha, alpha_replay_thresh, dim
        )
    ...
    print(
        f"Final memory sizes: live={mem_live_log[-1]}, "
        f"replay={mem_replay_log[-1]}, total={mem_total_log[-1]}"
    )
